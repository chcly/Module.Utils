...
using SearchTree     = ListBinaryTree<int, 0x40>;

void populate(SearchTree& tree, const int initial[], const int len)
{
    for (int i = 0; i < len; ++i)
            ...
            const int   removeListLen)
{
    ...
    {
        if (int v = removeList[i]; tree.findNonRecursive(v))
        {
            printf("Removing %i: = ", v);
...
GTEST_TEST(Utils, ListBinaryTree_002)
{
    ...
    constexpr int len   = std::size(List);
    constexpr int c2Len = std::size(C2);
